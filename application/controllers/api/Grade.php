...
class Grade extends REST_Controller {
    ...
    public function index_get() {
		...
		$data = array();
		if ($id) {
			$grade = $this->GradeManager->findById($id);
			//$type_indicateur = $this->Type_indicateurManager->findById($indicateur->type_indicateur_id);
			$data['id'] = $grade->id;
			$data['code'] = $grade->code;
			$data['libelle'] = $grade->libelle;
			$data['rang'] = $grade->rang;
			
		
		} else{
				$data = $this->GradeManager->findAll();
    ...
    public function index_post() {
        ...
        if ($supprimer == 0) {
            if ($id == 0) {
                ...
                $dataId = $this->GradeManager->add($data);
                if (!is_null($dataId)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => $dataId,
                ...
                $update = $this->GradeManager->update($id, $data);
                if(!is_null($update)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => 1,
            ...
            $delete = $this->GradeManager->delete($id);
            if (!is_null($delete)) {
                $this->response([
                    'status' => TRUE,
                    'response' => 1,
